...
const tasks = runCli("tasks", { ...admin, filters: { role: "admin", search: "__module_check_no_match__" } });
...
const modules = [
  ...
  },
  {
    ...
    checks: [
      ...
      ["ERP 设置是独立二级页面", html.includes('id="erpSettingsPage"') && renderer.includes('showPage("erpSettings")')],
      ["有 ERP 配置和手动同步按钮", hasButton("manualErpSyncBtn") && hasButton("saveErpSettingsBtn")],
      ["有 ERP 页面内同步结果区", html.includes('id="erpSyncResult"') && renderer.includes("setErpStatus")],
