...
function checkRenderSmoke() {
  const result = spawnSync(process.execPath, [path.join(ROOT, "scripts", "check-render.js")], {
    cwd: ROOT,
    encoding: "utf8",
  });
  if (result.status !== 0) {
...
function checkSecurity() {
  const result = spawnSync(process.execPath, [path.join(ROOT, "scripts", "check-security.js")], {
    cwd: ROOT,
    encoding: "utf8",
  });
  if (result.status !== 0) {
...
function checkDocs() {
  const result = spawnSync(process.execPath, [path.join(ROOT, "scripts", "check-docs.js")], {
    cwd: ROOT,
    encoding: "utf8",
  });
  if (result.status !== 0) {
...
checkModules();
checkBusinessTests();
console.log("启动检查通过：Electron、Python、关键文件、产品设计、真实渲染、界面绑定、安全体检、文档说明、模块体检、角色旅程、权限边界和核心业务测试均可用。");
